        ...
        <Modal :title="L('EditTenant')" :value="value" @on-ok="save" @on-visible-change="visibleChange">
            <Form ref="questionForm" label-position="top" :rules="tenantRule" :model="question">
                ...
                <FormItem label="解决方案">
                 ...
                 <!-- 格式化代码会导致控件失效！！！ -->
                <FormItem label="截图" v-if="question.imageArray!== undefined && question.imageArray!== null &&question.imageArray.length>0" id="viewer">
                    <viewer   style="height: 200px;">
                        <img :src="item" v-for="item in question.imageArray" :key="item"   height="100" />
                      </viewer>
                </FormItem>
            </Form>
            <div slot="footer">
    ...
    export default class EditTenant extends AbpBase {
        ...
        visibleChange(value: boolean) {
            if (!value) {
                ...
                this.question = Util.extend(true, {}, this.$store.state.question.editQuestion);
